...
func TestDevbrainCLI(t *testing.T) {
	...
	t.Run("import dry-run runs", func(t *testing.T) {
		fresh := t.TempDir()
		r := h.RunWith(clitest.RunOpts{Env: map[string]string{"HOME": t.TempDir(), "CODEX_HOME": t.TempDir()}}, "import", "--data", fresh)
	...
	t.Run("import wrote nothing (dry)", func(t *testing.T) {
		fresh := t.TempDir()
		h.RunWith(clitest.RunOpts{Env: map[string]string{"HOME": t.TempDir(), "CODEX_HOME": t.TempDir()}}, "import", "--data", fresh)
