#!/usr/bin/env bash
# devbrain — settings.json hook register/unregister tests (plus the
# session-start-context emitter and the stop-active field). These replaced the `jq`
# merges in install/uninstall/nightshift. The implementation lives in the Go binary
# now (`devbrain internal …`, backed by internal/jsonedit) — this drives the binary,
# not the legacy python lib.
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BIN="${DEVBRAIN_BIN:-$HERE/../devbrain}"
[ -x "$BIN" ] || BIN="$(command -v devbrain)" || { echo "skip: devbrain binary not built (go build -o devbrain ./cmd/devbrain)"; exit 0; }
command -v python3 >/dev/null 2>&1 || { echo "skip: python3 not installed (used only to read results)"; exit 0; }
...
"$BIN" internal register-hook "$S" UserPromptSubmit "" /bin/cap.sh
...
"$BIN" internal register-hook "$S" PostToolUse Bash /bin/gb.sh
...
"$BIN" internal unregister-hook "$S" /bin/cap.sh /bin/gb.sh
...
"$BIN" internal unregister-hook "$S" /bin/cap.sh
...
if "$BIN" internal register-hook "$S.bad" Stop "" /bin/x.sh 2>/dev/null; then rc=0; else rc=1; fi
...
ctx="$(printf '%s' 'say `gbrain search "x"` first' | "$BIN" internal session-start-context)"
...
check "stop-active true"  '[ "$(printf "%s" "{\"stop_hook_active\":true}"  | "$BIN" internal read-event stop-active)" = "true" ]'
check "stop-active false" '[ "$(printf "%s" "{\"stop_hook_active\":false}" | "$BIN" internal read-event stop-active)" = "false" ]'
check "stop-active absent" '[ -z "$(printf "%s" "{}" | "$BIN" internal read-event stop-active)" ]'
