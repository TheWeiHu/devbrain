...
const rcClaudeMeta = `{"type":"user","timestamp":"2026-01-02T03:04:05Z","cwd":"/repo","isMeta":true,"message":{"content":"scheduled watcher"}}
{"type":"assistant","timestamp":"2026-01-02T03:04:08.500Z","message":{"id":"m1","model":"claude-opus-4-5","usage":{"input_tokens":11,"output_tokens":22},"content":[{"type":"text","text":"Checked."}]}}
`
...
func TestResponseCapture(t *testing.T) {
	...
	t.Run("claude-meta-turn-overrides-hook-auto", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		path := writeFixture(t, "rc-claude-meta.jsonl", rcClaudeMeta)
		sidecar := filepath.Join(dir, "usage.jsonl")
		ResponseCapture(path, sidecar, "sess-meta", "2026-01-01T00:00:00Z", false, "")
		wantLine := `{"ts": "2026-01-02T03:04:08Z", "session": "sess-meta", "model": "claude-opus-4-5", "in": 11, "out": 22, "cache_create": 0, "cache_read": 0, "auto": true, "turn": "2026-01-02T03:04:05Z"}` + "\n"
		if b, err := os.ReadFile(sidecar); err != nil || string(b) != wantLine {
			t.Errorf("sidecar (err=%v):\n got: %q\nwant: %q", err, b, wantLine)
		}
	})
