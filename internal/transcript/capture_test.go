...
func TestResponseCapture(t *testing.T) {
	...
	t.Run("claude-with-sidecar", func(t *testing.T) {
		...
		wantLine := `{"ts": "2026-01-02T03:04:08Z", "session": "sess-1", "model": "claude-opus-4-5", "in": 11, "out": 22, "cache_create": 33, "cache_read": 44, "auto": true, "turn": "2026-01-02T03:04:05Z", "reasoning_effort": "", "service_tier": "", "parent_session": "", "subagent_count": 0}` + "\n"
	...
	t.Run("fallback-text-and-redaction", func(t *testing.T) {
		...
		wantLine := `{"ts": "2026-01-02T03:05:00Z", "session": "s2", "model": "claude-haiku-4-5", "in": 1, "out": 2, "cache_create": 0, "cache_read": 0, "auto": false, "turn": "2026-01-02T03:04:05Z", "reasoning_effort": "", "service_tier": "", "parent_session": "", "subagent_count": 0}` + "\n"
		if b, err := os.ReadFile(sidecar); err != nil || string(b) != wantLine {
			t.Errorf("sidecar (err=%v):\n got: %q\nwant: %q", err, b, wantLine)
		}
	})
	...
	t.Run("codex-last-turn", func(t *testing.T) {
		...
		wantLine := `{"ts": "2026-03-01T10:25:57Z", "session": "cx", "model": "gpt-5.2-codex", "in": 1000, "out": 300, "cache_create": 0, "cache_read": 4200, "auto": false, "turn": "2026-03-01T10:05:00Z", "reasoning_effort": "", "service_tier": "", "parent_session": "", "subagent_count": 0}` + "\n"
		if b, err := os.ReadFile(sidecar); err != nil || string(b) != wantLine {
			t.Errorf("sidecar (err=%v):\n got: %q\nwant: %q", err, b, wantLine)
		}
	})
