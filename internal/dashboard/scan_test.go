...
func TestCollapseAttachedPayloadsPreservesKinds(t *testing.T) {
	t.Parallel()
	c := defaultClassifier()
	recs := []*Prompt{
		{X: "PLEASE IMPLEMENT THIS PLAN:\n\n# user-approved plan", Kind: "human", Chars: 58, Words: 6},
		{X: "PLEASE IMPLEMENT THIS PLAN:\n\n# repeated agent payload", Kind: "repeat", Chars: 61, Words: 7},
	}
	collapseAttachedPayloads(c, recs)
	for i, r := range recs {
		if r.X != "PLEASE IMPLEMENT THIS PLAN:" || r.Chars != 27 || r.Words != 4 {
			t.Errorf("record %d not compacted correctly: %+v", i, r)
		}
	}
...
func TestClassify(t *testing.T) {
	...
	cases := []struct {
		...
		{"Hello memory agent, you are continuing to observe the primary Claude session.\n<observations>", false, "system"},
		{"This session is being continued from a previous conversation that ran out of context. The summary follows.", false, "system"},
