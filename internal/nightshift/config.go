...
// RuntimeFile is the dashboard-facing record of the provider choices for this
// run. It remains after shutdown so a stopped card is still attributable.
func (o Options) RuntimeFile() string {
	return filepath.Join(o.Repo, ".nightshift", "runtime.json")
}

// WriteRuntime records only operator-selected runtime controls. An inherited
// Codex model is labelled as such instead of guessing at config/profile state.
func (o Options) WriteRuntime() error {
	model, reasoning := "", ""
	if o.Mode == "codex" {
		model, reasoning = o.CodexModel, o.CodexReasoning
		if model == "" {
			model = "inherited"
		}
		if reasoning == "" {
	...
	b, err := json.Marshal(map[string]string{
		"backend": o.Mode, "model": model, "reasoning": reasoning,
	})
	if err != nil {
		return err
	}
	return os.WriteFile(o.RuntimeFile(), append(b, '\n'), 0o644)
}
