...
func TestDetectUsageLimitUsesClaudeResetClock(t *testing.T) {
	loc, _ := time.LoadLocation("America/Toronto")
	now := time.Date(2026, 7, 10, 8, 58, 0, 0, loc)
	sig := detectUsageLimit("You've hit your limit · resets 9:30am (America/Toronto)\n", 0, now, 5*time.Minute)
	want := time.Date(2026, 7, 10, 9, 31, 0, 0, loc)
	if !sig.limited || !sig.until.Equal(want) {
		t.Fatalf("signal = %+v, want until %s", sig, want)
	}
}
