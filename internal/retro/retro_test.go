...
func TestFocusHours(t *testing.T) {
	...
	prompts := func(ss ...string) []time.Time {
		var out []time.Time
		for _, s := range ss {
			out = append(out, mustT(s))
		}
		return out
	}

	t.Run("only gaps within the link threshold count", func(t *testing.T) {
		// 10:00->10:04 links (4m<=5); 10:04->10:20 does not (16m>5);
		// 10:20->10:23 links (3m). Agent runtime is irrelevant: only prompt ts.
		fs := focusHours(prompts(
			"2026-06-01T10:00:00Z",
			"2026-06-01T10:04:00Z",
			"2026-06-01T10:20:00Z",
			"2026-06-01T10:23:00Z",
		), 5)
		want := (4.0 + 3.0) / 60 // 4m + 3m, the >5m gap contributes nothing
		if got := fs.perDay["2026-06-01"]; math.Abs(got-want) > 1e-6 {
			t.Fatalf("day total = %.4f, want %.4f", got, want)
		}
	})

	t.Run("isolated prompt contributes nothing", func(t *testing.T) {
		fs := focusHours(prompts("2026-06-01T09:00:00Z"), 5)
		if fs.total != 0 {
			t.Fatalf("lone prompt total = %v, want 0", fs.total)
		}
	})

	t.Run("a linked gap crossing midnight is split by day", func(t *testing.T) {
		// two prompts 4m apart straddling midnight -> link credited to both days
		fs := focusHours(prompts(
			"2026-06-01T23:58:00Z",
			"2026-06-02T00:02:00Z",
		), 5)
		if got := fs.perDay["2026-06-01"]; math.Abs(got-2.0/60) > 1e-6 {
			t.Fatalf("Jun01 = %.4f, want %.4f", got, 2.0/60)
		}
		if got := fs.perDay["2026-06-02"]; math.Abs(got-2.0/60) > 1e-6 {
			t.Fatalf("Jun02 = %.4f, want %.4f", got, 2.0/60)
		}
	})

	t.Run("one long agent turn no longer inflates the day", func(t *testing.T) {
		// a single prompt whose response landed hours later credits nothing on
		// its own — the whole point of the prompt-driven metric.
		fs := focusHours(prompts(
			"2026-06-03T17:47:00Z", // then the agent ran for 7h; no more prompts
		), 5)
		if fs.total != 0 {
			t.Fatalf("long-agent-turn total = %v, want 0", fs.total)
		}
	})
}
