...
# Skip injected / synthetic "prompts" — these aren't real user input and only add
# noise: the title-generation prompt, the Conductor workspace preamble, slash-command
# expansions, and reminder blocks. Match a distinctive leading prefix after trimming
# leading whitespace. Keep in sync with the importer's filter (scripts/import.py).
_t="${prompt#"${prompt%%[![:space:]]*}"}"
case "$_t" in
  "<system_instruction>"*|"<system-reminder>"*|"<command-name>"*|\
