#!/usr/bin/env bash
# devbrain/drain — turn-completion marker (Stop hook).
#
# Fires when the agent finishes a turn. Appends ONE line per finished turn to the
# file named by $DRAIN_MARKER (the driver exports it before launching the worker),
# else <cwd>/.drain/turns.log. The driver watches this file's line count to know a
...
# Only active for drain workers (the orchestrator exports DRAIN_MARKER per worker).
# This makes the hook safe to register GLOBALLY: ordinary sessions no-op instantly,
# so we never litter .drain/ into unrelated repos, and the marker survives
# /continue's `git stash -u` (a worktree-local hook config would get stashed away).
[ -n "${DRAIN_MARKER:-}" ] || exit 0
...
marker="${DRAIN_MARKER:-$cwd/.drain/turns.log}"
