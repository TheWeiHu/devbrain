...
function fleet(r,i){
  ...
  return `<div class="ns-run">
    <div class="ns-head">
      ...
      ${(r.started||r.model)?`<div class="ns-started ns-caption" title="run ${esc(r.run_id||"")} · started ${esc((r.started||"").replace("T"," ").replace("Z"," UTC"))}${r.model?` · model ${esc(r.model)}`:""}">${r.started?`started ${esc(fmtStarted(r.started))}`:""}${r.started&&r.model?" · ":""}${r.model?`model ${esc(r.model)}`:""}</div>`:""}
