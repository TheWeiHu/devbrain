...
# Sample = the 5 most recently finished tasks: done (newest done_at first) + in-review.
...
| Branched from the base | `gh pr view <pr> --json commits` | the PR carries commits that are on `nightshift` but not on the base (e.g. `nightshift: merge …` subjects) — the branch was cut from the wrong base |
...
```markdown
# audit — <project> — YYYY-MM-DD (sample: 5 finished tasks)
